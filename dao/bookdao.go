...
//GetPageBooksByPrice 获取带分页的函数信息
func GetPageBooksByPrice(pageNo string, minPrice string, maxPrice string) (*model.Page, error) {
	...
	var totalRecord int64
	//执行
	r := utils.Db.QueryRow(sqlStr, minPrice, maxPrice)
	r.Scan(&totalRecord)
	//设置每页只显示四条记录
	var pageSize int64
	pageSize = 4
